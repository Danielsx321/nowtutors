...
/**
 * The retrying Realtime channel every subscription in the app uses (SPEC §8).
 *
 * Moved here unchanged from `use-session-requests.ts` in Phase 9 Part 1, so the
 * messaging hooks subscribe with the same JWT-before-subscribe prelude, connect
 * watchdog and bounded backoff that were proven against the free-tier tenant
 * sleep for instant requests, instead of a second copy that could drift.
 */
 ...
 */
export function useRetryingChannel(
