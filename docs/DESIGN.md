...
The tutor card is the product. Its anatomy, in order (v3, design round 3 Part D, after Oranum's expert card): a white card with 10px padding and a 22px radius; a landscape 16:10 photo inset at 15px (`rounded-photo`) with the on-air ring when the tutor takes instant requests; the live chip on the photo, top-left, on a white pill, only when live or broadcasting (an offline tutor shows **no** status text: absence is the signal); favourite top-right; the name in the display face with the country, by name, on a solid translucent navy strip (`bg-ink/75`, never a gradient) along the photo's bottom edge; under the photo the stacked rate ("45 cr" over "≈ $60") on the left and up to three subject chips on the right; one full-width action. `primary` (blue) "Request now" for instant-available, `outline` "Watch live" for broadcasting, `ink` "Book a session" otherwise. Experience and sessions are not on the card any more; they sit on the profile's identity line (Part E). The card rises 3px with `shadow-lift` on hover, never under reduced motion. Below `md` the `row` variant keeps the same states in a horizontal card with the first two subject chips.

The profile (v3, Part E) opens on the stage: the photo at 16:9 in a 26px panel radius (ring outside a ground-coloured gap when instant-available, the live chip on it), or the broadcast player in a dark island for a signed-in student when the tutor is live, or the dimmed photo with a LIVE chip and a centred orange "Watch live" for everyone else. Under it the identity block: the name at display size with the live chip beside it, the pitch, country and languages, the sessions-and-experience line (the old proof row, in words), the subject chips. Then white blocks (About, This week, Background) and the 26px sticky panel on the right, unchanged.
