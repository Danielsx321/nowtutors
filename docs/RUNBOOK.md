...
already-running script (see "`NODE_EXTRA_CA_CERTS` is set by a wrapper
process" in DECISIONS.md).
...
- **`pnpm test:e2e`:** wrapped internally — see "Running the E2E suite" below.
...
- All `db:*`/`db:*:test` scripts run through `scripts/with-ca-certs.mjs`, which
  ...
  **Fallback:** if your system CA bundle is at a different path, export
  ...
  quirk, not limited to `db:*` scripts — see "Node's CA trust store does not
  complete chains the system bundle does" above for the full picture,
  including the `playwright install` symptom. CI and Vercel are unaffected.
...
- **Why a production build and not `next dev`:** see DECISIONS.md — under
  `next dev` every route compiles on first request, which inflated every
  timeout in the spec into latency cover rather than a real budget.
