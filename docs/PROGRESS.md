...
the live count and faces, summary cards (next session, credits ≈ $, this month), upcoming sessions carousel
(opens the booking page), right column (greeting with the balance, hours learned per month, your tutors with
Request or Book again) and the bookings table across the full width. Topbar search goes to `/tutors?q=`,
...
signed in, and a menu sheet below `md`. `SiteFooter` is the approved ink footer: brand, the real live count
(read server-side, "No tutors live right now" at zero), two actions, Learn / Teach / Account columns and the
trust pair. `src/lib/routes.ts` lists every route the app serves and both nav tests fail on an href that isn't
in it, which turns SPEC §10.3 from a rule into a check. `/tutor/pending-approval` moved into its own route
group to escape the tutor app shell (guards unchanged). `PublicHeader` and `PublicFooter` are deleted.
**Fixed on the way:** `getLiveStrip` shared one drizzle `and(...)` across two concurrent queries, so parameter
...
checks those signed in on production; Part 4 (live moments) starts from `main`.
...
clean, 677 unit, 58 DOM, build passed. **Next:** Part 3 (public and student surfaces) starts
...
project. No migration, no prod step. **Next:** Part 3 (public and student surfaces, led by the tutor card) starts from `main`.
...
**Daniels runs them** against the test project (they sign in with the seeded password):
`pnpm test:e2e tests/e2e/messaging-realtime.spec.ts tests/e2e/broadcast-two-viewers.spec.ts`. SPEC §16 marks Phase 9
COMPLETE only with both passing.
...
settled** (a broadcasting tutor can't take instant requests), so Part 3 is unblocked. **Post-merge production step: DONE 2026-09-15.** Signed-in live check is batched (live-test item 11).
...
blocks Part 3 only.**
**Migration `drizzle/0018`** removes every client write on `conversations`, `messages`, `broadcasts` and
