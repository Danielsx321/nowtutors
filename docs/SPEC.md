...
- **A student may hold at most two `pending_payment` bookings at once** (launch fix R3, 2026-09-26): `createScheduledBooking` counts the student's unpaid holds younger than the hold window inside its transaction and refuses a third (`lib/bookings/payment-holds.ts`). Without it one account could hold every open slot on a tutor's calendar for free and renew the holds every 20 minutes.
...
> **How identity is established (2026-09-21, `perf-auth-local-claims`).** `getUser()` in the guards and the middleware read the access token's claims with `auth.getClaims()`: the ES256 signature is verified locally against the project's public key, and expiry is checked. No call to the auth server per request. A session revoked before its token expires (sign out elsewhere, deleted auth user) is therefore honoured for up to the token lifetime, one hour. Suspension and role are unaffected, since both are read from `profiles` on every request. The email-verification gate (`requireVerifiedEmail`, `requireApiVerifiedEmail`: bookings, paid requests, credit purchase, going live) still calls the auth server through `getVerifiedUser()`. The Supabase access-token lifetime must stay at one hour or less.
...
/                                  Home landing (amended 2026-09-18, live-globe rebuild Part C): hero with
                                   the real live count and a turning globe with a dot per country where a
                                   tutor is live, proof strip, app screens (#how), tutors live now (or
                                   "Popular tutors" when nobody is), three steps, proof wall, closing block.
                                   Any result-changing browse key (subject, lang, price, live, sort,
                                   minRating, cursor) permanently redirects to /tutors with the same query,
                                   so pre-Part-C shared links keep working.
/tutors                            Browse (Section 7.2): search pill, filter chips, count and sort, grid
/tutors/[slug]                     Tutor profile (amended 2026-09-18, live-globe Part D): header, proof row,
                                   About, Subjects, "This week" (open start times per day in the viewer's
                                   timezone, from the same calendar the panel books from), Background, and
                                   a sticky panel: rate, "Start now" (if instant-available), "Book a
                                   session", Message, trust lines. No reviews until after launch (§18)
...
/dashboard                         (amended 2026-09-18, live-globe Part E) Teal banner with the live count,
                                   summary cards (next session, credits, this month), upcoming sessions
                                   carousel, right column (greeting, hours per month, your tutors), and
                                   your bookings table across the full width
...
/tutor                             (amended 2026-09-19, live-globe Part F) Go-live banner sharing the topbar
                                   switch's state, summary cards, today's sessions, profile checklist,
                                   earnings in three stages, credits per month, recent students, upcoming
                                   bookings table
/tutor/bookings[/[id]]
...
6. Admin approves in `/admin/tutors` → email to tutor → tutor can now appear in search and go live. **A tutor must have a profile photo to be approved** (amended 2026-09-19, live-globe Part G): onboarding and the profile editor require one, and `approveTutor` refuses without one on the server.
...
Layout (amended 2026-09-18, live-globe rebuild Part C; pages.html, Browse): kicker and title, a search pill that searches subjects (the browser suggests names from the active list; Search picks the best match and sets `?subject=`, keeping other filters; text matching no subject says so), a chip row (Live now first, All subjects, the five most-taught subjects, then Price and Language as dropdown chips) with the result count and sort beside it, and the card grid with the no-show promise tile after the third card. There is no side rail and no phone-only filter sheet: every filter is a chip, and the chip row scrolls sideways on phones. `minRating` stays dormant as above.

Card contents (amended 2026-09-16, design overhaul Part 3, and 2026-09-18, live-globe Part D; DESIGN.md "Cards"): photo inset (initials fallback), a "Live now" chip and the green on-air ring only when in `live_tutors` for instant sessions AND `accepts_instant`, a "LIVE" chip linking to the broadcast when broadcasting, nothing at all when offline; favourite heart; name and country (by name); headline; the first subject as a tag; proof row (Experience, Sessions, Rate with the "≈ $" anchor from the direct-pay basis package); one action: teal "Request now" (to the profile's Start now panel), outline "Watch live", or ink "Book a session". No rating until reviews exist (§18). A `row` variant is used below `md`. The filter bar leads with a "Live now" chip and shows the result count across all pages.
...
Also implement `POST /api/webhooks/paypal` handling `PAYMENT.CAPTURE.COMPLETED`, `PAYMENT.CAPTURE.DENIED`, `PAYMENT.CAPTURE.REFUNDED` and, since the launch fixes of 2026-09-26, `PAYMENT.CAPTURE.REVERSED` (a chargeback, recorded as a full refund; M10). **Verify the webhook signature** against `PAYPAL_WEBHOOK_ID`, sending PayPal the event **as the exact bytes it transmitted**, never re-serialised (M8). A late `COMPLETED` on a payment already `refunded` writes nothing and credits nothing (M4). Every order carries our `payments.id` as `custom_id`, and an event is also matched on it, so a row whose order id was never stamped is still settled (M11). The webhook is a backstop for users who close the tab mid-capture — it must be idempotent, which the unique `(type, reference_id)` ledger index provides for free.
...
>
> **Direct-pay pricing basis (amended 2026-09-26, launch fix M6).** The direct-pay order charges the price **pinned on the booking** (`bookings.price_credits`, written when the slot was taken), the same number the tutor's earnings are split from. It is no longer re-derived from the tutor's current rate: a rate change inside the payment hold changed what the student paid but not what the tutor earned. The original text follows for the record. `price_credits = sessionPriceCredits(tutor's current rate, duration)`
...
- **Earnings are owed by the booking's state, not by who closed it (code review 2026-09-20).** `complete-sessions` writes the `held` row for every booking that is `completed` or `no_show_student` and has no `tutor_earnings` row, read from the database on each run (`writeOwedEarnings`, at most 500 per run, oldest first). It used to pay only the bookings that same run had transitioned, held in memory. That missed every instant session closed by `getSessionState` at the deadline, by `endSession`, or by the token route (they are `completed` before a sweep sees them), so those tutors were never paid; and a run that died between its transitions and its insert left rows that no later run could find. The read and the insert are one transaction with the booking rows locked `FOR UPDATE SKIP LOCKED`, which serializes with admin force-cancel (it locks the booking first too): a cancel that wins leaves nothing to pay, a cancel that loses finds the `held` row and reverses it. A booking with NULL `price_credits` is withheld and reported on every run. The first run after this shipped pays the backlog.
...
**Amended 2026-09-15 (design overhaul, "Phase 9.5").** The earlier text of this section (the brand-continuity rule, the ink-shell model, the purple/gold/ink token set) is superseded. The name and the wordmark stay; the visual identity is new. Direction **"On Air"**: a white canvas, ink text and ink primary buttons, a deep teal accent (the brand colour, used lightly) for links, selected states and focus, one green for live, Funnel Display and Funnel Sans. **Amended 2026-09-16 (Noora's answers):** the logo is her own "nowtutors" artwork as a vector, teal replaces plum, and yellow is gone; green is the live signal. The full contract, with the reasoning and the banned list, is **`docs/DESIGN.md`**; this section is the summary that binds the build.
...
**Dark islands.** Interactive content on a dark background sits inside a `.theme-dark` scope, never on a bare `surface-inverse`, so the single focus ring re-resolves to the light teal (9.15:1) instead of the deep teal on near-black (1.46:1). The footer is a dark island; a tooltip is not.

### 10.2 Component inventory

Primitives (`src/components/ui/`): `Button` (primary / **highlight** / **ink** / **outline** / ghost / **live** / danger × sm / md / lg / icon, loading, asChild; `secondary` is a deprecated alias of `outline`), `Input`, `Textarea`, `Select`, `Checkbox`, `Radio`, `Switch`, `DatePicker`, `TimePicker`, `Label`, `FieldError`, `Card` (`interactive`), `StatCard`, `Badge` (neutral / accent / live / success / warning / danger / solid), **`LiveChip`** (replaces `LivePill`: always a text label, "Live now" or "LIVE" with an optional viewer count), **`OnAirRing`** (the green ring for instant-available tutors, `still` variant), **`StatRow`** (Experience, Sessions, Rate), **`Money`** (credits with the "≈ $" anchor from the basis package), **`AlertDialog`** (money and destructive confirms; `role="alertdialog"`), **`Progress`** (linear), `ProgressRing` (the 60-second countdown), `Avatar` (initials fallback on `surface-muted`), `RatingStars` (in the inventory, unrendered until reviews exist, §18), `SubjectChip`, `Tabs`, `Modal`, `Drawer`, `Toast`, `Tooltip`, `DropdownMenu`, `Pagination`, `EmptyState`, `Skeleton`, `Spinner`, `PriceTag`, `CreditBalance`, `Table` (`numeric` cells), `Breadcrumb`, `Alert`. Plus `Wordmark` in `components/layout/`, alongside the site shell: **`SiteHeader`** (wordmark, pill nav with the current page as a teal pill, Log in / Sign up signed out or the avatar and a link to the role home signed in, and a menu sheet below `md`), **`SiteFooter`** (brand, the live count read server-side, two actions, the Learn / Teach / Account columns and the trust pair, as a dark island) and **`SiteShell`** (header + main + footer, which reads the viewer and the live count itself).

Composed: `TutorCard`, `BookingCard`, `SlotPicker`, `AvailabilityGrid`, `MessageBubble`, `ConversationListItem`, `TransactionRow`, `VideoTile`, `SessionControlBar`, `IncomingRequestModal`, `WaitingForTutorModal`. Part 2 added `BottomNav` (the mobile bottom bar) and `UnreadCountProvider` (one Realtime channel for both badge sites); Parts 3 to 6 add the room pieces (`Lobby`, `ControlBar`, `SidePanel`, `ConnectionBanner`, `SessionEnded`), the tutor pieces (`TodayFeed`, `EarningsStages`, `PayoutInFlight`) and the admin `DataTable`; each lands in its part's SPEC amendment.
...
**The live signal rule.** Two states, one green family, mutually exclusive (§7.8): instant-available is the green `OnAirRing` plus a "Live now" `LiveChip` plus the `live` "Request now" button; broadcasting is a "LIVE" `LiveChip` with the viewer count and no ring. The chip is always text; a bare dot is never the indicator. No red for live. An offline tutor shows no status text.
...
**Home and Browse (Part C, 2026-09-18).** `/` is the home landing and `/tutors` is browse (§6, §7.2). "How it works" is in the header's pill nav and the footer's Learn column, pointing at `/#how`; an anchor link is never marked as the current page. Every number on the home page is read on the request and nothing is padded: a proof-wall tile whose number is zero is left out, the floating tutor cards over the globe appear only when two or more live tutors have photos, and the "photo-checked" claim waits for Part G.
...
*Status:* **Part 1 (foundation) merged 2026-09-15** (PR #74, `be6a209`). **Part 2 (shell) merged 2026-09-15** (PR #76, `c2df144`). **Brand update (Part 2.5) merged 2026-09-16** (PR #78, `004267b`) after Noora's answers: teal accent, yellow removed, green live signal, SVG wordmark. **Part 3 (public and student) merged 2026-09-16** (PR #80, `aca6172`). **Part 4 (live moments) merged 2026-09-17** (PR #82, `f643587`); screen share, in-session chat and tutor notes moved to their own phase. **Parts 5 and 6 are superseded**: on 2026-09-17 Daniels judged the result too plain and approved a fuller "live globe" direction, so the remaining work is the rebuild in workspace `plans/2026-09-17-nowtutors-live-globe-rebuild.md` (Parts A to I, one PR each, from `main`). The acceptance criteria above still apply to it. **Rebuild Part A (foundation v2) merged 2026-09-17** (PR #84, `33a7e49`): the v2 tokens, button variants, radii and the banned-tells tests. **Part B (site header and full footer everywhere public) merged 2026-09-18** (PR #86, `b4a1e74`), with `src/lib/routes.ts` making §10.3 testable. **Part C (Home and Browse split, the globe) merged 2026-09-18** (PR #88, `f0275e8`). **Part D (tutor card and profile v2) merged 2026-09-18** (PR #90, `7b6381b`). **Part E (app shell v2 and student dashboard) merged 2026-09-18** (PR #92, `ea956f8`). **Part F (tutor dashboard and surfaces) merged 2026-09-19** (PR #94, `c406101`); full-width app pages and loading feedback merged (PR #95, `f590f08`). **Part G (admin dashboard, photo rule, cleanup) merged 2026-09-19** (PR #96, `b4571c6`). **Part H (live moments) merged 2026-09-19** (PR #97, `d4ebf69`). **Part I (acceptance):** PR 1 merged 2026-09-19 (PR #101, `2154d9f`); PR 2 is PR #104.
*Acceptance, checked 2026-09-20 on Daniels' Mac against the test project:* every E2E spec passes (E2E 3, 4, 6, 7 and the 20-check design smoke; 23 passed in the full run, and the two specs interrupted by a network change mid-run passed on their own straight after). The alias grep finds nothing real: run literally it matches `shrink-0` 66 times, so use `grep -rE '(^|[^a-z])(ink-|gold-|purple-|live-4|live-5|gray-)' src`, which returns nothing. Lighthouse accessibility is 100 on `/`, `/tutors`, a profile and `/login`, with no failing audits (`/login` stands in for `/dashboard`, which needs a signed-in session Lighthouse doesn't have; the dashboards are covered by the design smoke). The before-and-after set (five desktop pairs and a phone page; the "before" half is a real build of `ef5bcf6`, the commit before Part 1) was approved by Daniels on 2026-09-20 for him to send to the client. **COMPLETE 2026-09-20.** Part I PR 2 merged (PR #104, `f97c6f4`). Functions moved to `cdg1`, next to the database (PR #105, `9ee9bf9`).

**Phase 9.6 — Design round 3 (Oranum structure, InstaEDU colours).** Added 2026-09-26 at the client's request after two references: the page structure of oranum.com and the colours of the InstaEDU tutoring site. Six PR-sized parts, no migration, no dependency, no money or booking rule change: (A) the v3 palette through the token roles; (B) the navy header and footer; (C) home and browse on one layout with the search band, the sidebar and the live row, the v2 home parked; (D) the landscape tutor card; (E) the profile stage; (F) acceptance. Plan: workspace `plans/2026-09-26-nowtutors-design-round-3.md`. Dashboards and rooms keep their v2 layout in the v3 colours.
*Accept:* every E2E spec passes on Daniels' run; `tests/unit/design-tokens.test.ts` passes with no floor lowered; Lighthouse accessibility 90+ on `/`, `/tutors`, one profile and `/login`; the client has the before-and-after set.
*Status:* **COMPLETE 2026-09-26.** Part A merged (PR #111, `fa77714`), Part B (PR #113, `15bb615`), Part D (PR #114, `2000c5d`), Part C (PR #115, `3ed2ab7`), Part E (PR #116, `e98d81a`). Noora approved the two mockups before Parts C to E were built. *Acceptance, checked 2026-09-26 on Daniels' Mac against the test project:* all five E2E specs pass in one run (25 passed, 11.4 min); token test 105/105 with the InstaEDU fills deepened to clear the floors rather than the floors lowered; Lighthouse (desktop preset, local production build) accessibility 100 on all four pages with no failing audits, performance 84 / 84 / 94 / 100. The before-and-after set is in the workspace at `outputs/nowtutors/before-after-round-3/` ("before" is round 2's approved set, the state Noora last saw).
