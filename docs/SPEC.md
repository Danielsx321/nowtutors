...
Spacing on a 4px grid. Container max-width 1200px, page gutter 20px mobile / 24px desktop (desktop gutter tightened from 32px in the density pass). **`/` (the browse page) is full-bleed by design, not centered in `container-page`** (`feat/browse-page-ink-theme`): the Bubble build pins its sidebar to the left edge, and centering it in a 1200px container reintroduced the large edge margins the coded rebuild was fixing. The browse page instead uses `w-full` with a small edge gutter (`px-4` mobile / `px-6` desktop) so the filters rail sits near the left edge and the tutor grid extends toward the right edge. This is scoped to `/` only — the `container-page` utility and its 1200px max-width are unchanged for every other page.
