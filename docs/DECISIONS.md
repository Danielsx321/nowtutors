...
`tests/e2e/presence-ungraceful-exit.spec.ts` now runs against a **compiled production build**
...
tooling permission on one machine, not a rule every clone of this repo should inherit. **The
gitignore entry for it was added in this same session, because the file was NOT previously
ignored** — before this, `.claude/settings.local.json` would have been picked up by a broad `git add
.`/`git add -A` and could have been committed by accident, silently turning a machine-local
permission into something that looked like committed repo policy. Caught and fixed before the
permission file was created, not after.
