...
13. **Browser check blocked on the test database.** The Supabase test pooler was timing out even for `select 1` from a standalone script (connect timeouts, statement timeouts, a 17-second success), while Supabase's REST API answered in 0.4 s. So the pages were checked on the PR's Vercel preview instead (PR description).
