-- Schedule the session-completion sweep (SPEC §7.11, §12) — Supabase pg_cron.
--
...
-- day, which cannot honour the `*/15 * * * *` §12 asks for here. Until this is
-- scheduled, nothing in the deployed app transitions a booking to `completed`
-- and no `tutor_earnings` row is ever written — see docs/RUNBOOK.md for the
-- consequence of leaving this unscheduled. This is unlike `sweep-presence` and
-- `expire-requests`, which are tidy-up on top of correctness enforced
-- elsewhere: `complete-sessions` is the only writer of `tutor_earnings` in the
-- codebase, and nothing else in the deployed app performs its transition when
-- both parties have walked away from a session.

-- The job. Every fifteen minutes, per SPEC §12.
-- Unschedule first so re-running this snippet updates rather than duplicates.
select cron.unschedule('complete-sessions')
 where exists (select 1 from cron.job where jobname = 'complete-sessions');
...
--                    (`{"ok":true,"job":"complete-sessions","completed":N,
--                    "noShowTutor":N,"noShowStudent":N,"earningsCreated":N,…}`):
...
-- Teardown: select cron.unschedule('complete-sessions');
